...
//      $this->error( 'Invalid cell index ' . $cell );
    ...
    public function readRowsEx($worksheetIndex = 0, $limit = 0)
    {
        if (!$this->rowsExReader) {
            $exReaderPath = __DIR__ . '/SimpleXLSXEx.php';
            if (is_readable($exReaderPath)) {
                require_once $exReaderPath;
            }
            if (class_exists(__NAMESPACE__ . '\\SimpleXLSXEx')) {
                $this->rowsExReader = new SimpleXLSXEx($this);
            }
        }
        if ($this->rowsExReader) {
            yield from $this->rowsExReader->readRowsEx($worksheetIndex, $limit);
            return;
        }

        foreach ($this->readRows($worksheetIndex, $limit) as $row) {
            $exRow = [];
